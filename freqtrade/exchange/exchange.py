...
class Exchange:
    ...
    async def _async_fetch_trades(
        self, pair: str, since: int | None = None, until: int | None = None, params: dict | None = None
    ) -> tuple[list[list], Any]:
        """
        ...
        try:
            ...
            if params:
                ...
                trades = await self._api_async.fetch_trades(pair, since=since, until=until, limit=trades_limit)
    ...
    async def _async_get_trade_history_time(
        ...
        while True:
            try:
                t, since_next = await self._async_fetch_trades(pair, since=since, until=until)
                if t:
